...
function YoutubeMp3Downloader(options) {
    ...
    self.downloadQueue = async.queue(function (videoId, fileName, callback) {

        self.performDownload(videoId, fileName, function(err, result) {
            callback(err, result);
        });
...
YoutubeMp3Downloader.prototype.download = function(videoId, fileName) {
    ...
    self.downloadQueue.push(videoId, fileName, function (err, data) {
        if (err) {
            self.emit("error", err);
        } else {
...
YoutubeMp3Downloader.prototype.performDownload = function(videoId, fileName, cb) {
    ...
    ytdl.getInfo(videoUrl, function(err, info){
        ...
        fileName = self.outputPath + '/' + fileName + '.mp3';
