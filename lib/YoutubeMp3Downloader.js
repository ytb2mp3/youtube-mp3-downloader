...
class YoutubeMp3Downloader extends EventEmitter {
  ...
  async performDownload(task, callback) {
    ...
    let exec_one = (httpResponse)=>{
      exec_one = (httpResponse)=>{return}
      
      let file_length = parseInt(httpResponse.headers['content-length']);
      if(httpResponse.headers['content-range']){
        file_length = parseInt(httpResponse.headers['content-range'].split('/')[1])
      }
    ...
    //also called for reconnections
    stream.on('response', function(httpResponse) {
      exec_one(httpResponse);
    });
